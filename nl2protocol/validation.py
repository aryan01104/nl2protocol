...
class ConfigValidator:
    ...
    def validate(self, config: Dict[str, Any]) -> ValidationResult:
        ...
        if not any(e.category == "schema" and "Missing required" in e.message for e in errors):
            ...
            errors.extend(self._validate_labware(config))
    ...
    def _validate_schema(self, config: Dict[str, Any]) -> List[ValidationError]:
        ...
        if "labware" not in config:
            errors.append(ValidationError(
                category="schema",
                message="Missing required field 'labware'",
                path="labware"
            ))
        elif not isinstance(config["labware"], dict):
            errors.append(ValidationError(
                category="schema",
        ...
        elif not isinstance(config["pipettes"], dict):
            errors.append(ValidationError(
                category="schema",
        ...
        if isinstance(config.get("labware"), dict):
        ...
        if isinstance(config.get("pipettes"), dict):
    ...
    def _validate_labware(self, config: Dict[str, Any]) -> List[ValidationError]:
        """Validate that load_names are valid Opentrons labware."""
        errors = []
        ...
        for name, lw in labware.items():
            ...
            if load_name and load_name not in self._valid_labware:
                errors.append(ValidationError(
                    category="labware",
