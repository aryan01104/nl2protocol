...
- **Self-refinement on detected gaps** (Stage 3 — focused per-gap LLM calls when deterministic fills can't resolve a missing field): follows Chapter 5 of the same book on self-refinement as inference-time scaling. Implemented as `LLMSpotSuggester` in [`nl2protocol/gap_resolution/suggesters.py`](nl2protocol/gap_resolution/suggesters.py) within the unified gap-resolution orchestrator (see ADR-0008). Replaced the earlier `extractor.refine` whole-spec rewrite path in PR3b.
